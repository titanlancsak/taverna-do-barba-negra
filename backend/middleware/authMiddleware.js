...
async function requireAuth(req, res, next) {
  ...
  try {
    ...
    const result = await pool.query('SELECT is_banned, ban_reason FROM users WHERE id = $1', [decoded.userId]);
    if (result.rows.length === 0) {
      return res.status(401).json({ error: 'Invalid or expired token' });
    }
    if (result.rows[0].is_banned) {
      return res.status(403).json({
        error: 'このアカウントは停止されています' + (result.rows[0].ban_reason ? '：' + result.rows[0].ban_reason : ''),
        banned: true
      });
    }
